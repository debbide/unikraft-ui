...
function formatPushedAt(value: string) {
...
export default async function ImagesPage() {
  const [{ images, error }, { jobs, error: jobsError }] = await Promise.all([
    listTemporaryImages(),
    listConversionJobs(),
  ]);
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">临时镜像</h1>
        <p className="mt-2 text-muted-foreground">
          管理 Docker 转换生成的 Unikraft Cloud 镜像。
        </p>
      </div>
      <ConvertImageForm />
      <Card>
        ...
        <CardContent>
          <ConversionJobs initialJobs={jobs} />
          {jobsError && (
            ...
            <TableBody>
              {images.map((image) => (
                <TableRow key={image.reference}>
                  ...
                  <TableCell className="whitespace-nowrap text-muted-foreground">
                    {formatPushedAt(image.createdAt)}
                  </TableCell>
                  <TableCell className="text-right">
                    <DeleteTemporaryImageButton
